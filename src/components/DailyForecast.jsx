...
const DailyForecast = ({ faren, dark }) => {
  ...
  return (
      <>
        {dark ? (
          <>
            <h2 style={{ textAlign: 'left', marginTop: 30, color: "#FFFFFF" }}>5-day forecast</h2>
            {daily.filter((_, idx) => idx % 8 === 0).map(hour => (
              <div style={stylesDark.dailyCard} key={hour.dt_txt}>
                <h4 style={{ textAlign: 'left', marginRight: 0 }}>{getDayAbbreviation(hour.dt_txt)}</h4>
                <div style={stylesDark.main}>
